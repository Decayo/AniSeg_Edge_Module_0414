...
if __name__ == '__main__':
    with Engine(custom_parser=parser) as engine:
        ...
        unsupervised_train_loader_0, unsupervised_train_sampler_0 = get_train_loader(engine, AniSeg, \
                    train_source=config.unsup_source, unsupervised=True, collate_fn=mask_collate_fn)
        unsupervised_train_loader_1, unsupervised_train_sampler_1 = get_train_loader(engine, AniSeg, \
        ...
        for epoch in range(engine.state.epoch, config.nepochs):
            ...
            for idx in pbar:
                ...
                mask_params = unsup_minibatch_0['mask_params'] # cut & paste mask
                ...
                with torch.no_grad():
                    with torch.cuda.amp.autocast():
                        ...
                        logits_u0_tea_1 = model(unsup_imgs_0, step=1)
                        logits_u1_tea_1 = model(unsup_imgs_1, step=1)
                        logits_u0_tea_1 = logits_u0_tea_1[0][-1].detach()
                        logits_u1_tea_1 = logits_u1_tea_1[0][-1].detach()
                        # Estimate the pseudo-label with branch#2 & supervise branch#1
                        logits_u0_tea_2 = model(unsup_imgs_0, step=2)
                        logits_u1_tea_2 = model(unsup_imgs_1, step=2)
                        logits_u0_tea_2 = logits_u0_tea_2[0][-1].detach()
                        logits_u1_tea_2 = logits_u1_tea_2[0][-1].detach()
                with torch.cuda.amp.autocast():
                ...
                # the mask pixels are either 1 or 0
                    # cut - paste 
                    # size ? 
                    h, w = batch_mix_masks.size(2), batch_mix_masks.size(3)
                    logits_u0_tea_1 = F.interpolate(input=logits_u0_tea_1, size=(h, w),
                                       mode='bilinear', align_corners=True)
                    logits_u1_tea_1 = F.interpolate(input=logits_u1_tea_1, size=(h, w),
                                       mode='bilinear', align_corners=True)
                    ...
                    logits_u0_tea_2 = F.interpolate(input=logits_u0_tea_2, size=(h, w),
                                       mode='bilinear', align_corners=True)
                    logits_u1_tea_2 = F.interpolate(input=logits_u1_tea_2, size=(h, w),
                                       mode='bilinear', align_corners=True)
                    ...
                    s1_ce29_group = model(unsup_imgs_mixed, step=1)
                    # Get student#2 prediction for mixed image
                    s2_ce29_group = model(unsup_imgs_mixed, step=2)
                    s1_ce29_group =  F.interpolate(input=s1_ce29_group[0][-1], size=(h, w),
                                       mode='bilinear', align_corners=True)
